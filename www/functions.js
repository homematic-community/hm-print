...
function _printWindow(element) {
  var printWin = window.open(
    "", "printWindow",
    "toolbar=yes, menubar=yes, scrollbars=yes, resizable=yes,location=yes, directories=yes, status=yes"
  );

  var content = "";
  if (document.getElementById(element) != null)
    content = document.getElementById(element).outerHTML;
  ...
  if (printWin.document.getElementById("printDiv") == null)
    printWin.document.write(' \
      ...
      <link rel="stylesheet" type="text/css" href="/addons/print/druckansicht.css" /> \
      </head><body><br> \
      <div class="FooterButton" onclick="javascript: window.stop(); window.print();" \
         style="padding-left: 20px; margin-left: 10px; width: 90px;">DRUCKEN</div> \
      <div id="printDiv">' + content + '</div><br> \
      <div class="FooterButton" onclick="javascript: window.stop(); window.print();" \
         style="padding-left: 20px; margin-left: 10px; width: 90px;">DRUCKEN</div> \
      </body></html>'
    );
  else printWin.document.getElementById("printDiv").outerHTML = content;
...
function _printPrograms(scope) {
  var printProgramsWin = window.open(
    "", "printProgramsWindow",
    "toolbar=yes, menubar=yes, scrollbars=yes, resizable=yes,location=yes, directories=yes, status=yes"
  );

  if (printProgramsWin.document.getElementById("printDiv") == null)
    printProgramsWin.document.write(' \
      <html xmlns="http://www.w3.org/1999/xhtml"><head> \
      <meta http-equiv="Content-Type" content="text/html; charset=iso-8859-1" /> \
      <link rel="stylesheet" type="text/css" href="/addons/print/druckansicht.css" /> \
      ...
      <script type="text/javascript" src="/addons/print/functions.js"></script> \
      </head><body><br> \
      <div class="FooterButton" onclick="javascript: window.stop(); window.print();" \
         style="padding-left: 20px; margin-left: 10px; width: 90px;">DRUCKEN</div> \
      <div id="header" lang="' + jQuery("#header").attr("lang") + '" style="height: 0px;"></div> \
      <div id="printDiv"></div><br> \
      <div class="FooterButton" onclick="javascript: window.stop(); window.print();" \
         style="padding-left: 20px; margin-left: 10px; width: 90px;">DRUCKEN</div> \
      <script>SessId = "' + SessionId + '"; loadTextResource();' + scope + '</script> \
      </body></html>'
    );
}
  ...
  for (var i = 0; i < programs.length; i++) {
    if (programs[i] != "") {
      ...
      document.getElementById("printDiv").innerHTML +=
        '<div id="prgcontent' + pid + '" style="page-break-before:always">';

      var p  = '<table id="ProgramTable' + pid + '" class="filterTable tTable j_translate" border="0" cellpadding="0" cellspacing="0">';
          p += '  <thead>';
          p += '    <tr>';
          ...
          p += '      <td class="tBodyCell bgWhite">';
          p += '        <input type="checkbox" id="p' + pid + '_systeminternal" ' + (parray[3] ? "true" : "false") + '" />';
          p += '        <label for="p' + pid + '_systeminternal">system intern</label>';
          p += '      </td>';
          p += '    </tr>';
          p += '    <tr>';
          p += '      <td colspan="5">';
    ...
    if (doc.childNodes[cn].tagName) {
      ...
      if (doc.childNodes[cn].tagName.toLowerCase() == "select")
        doc.childNodes[cn].outerHTML =
          "<b><u>" +
          doc.childNodes[cn].options[doc.childNodes[cn].selectedIndex].text +
          "</u><b>";
      else if (
        doc.childNodes[cn].tagName.toLowerCase() == "input" &&
        doc.childNodes[cn].type.toLowerCase() == "text"
      )
        doc.childNodes[cn].outerHTML =
          "<b><u>" + doc.childNodes[cn].value + "</u></b>";
      else if (doc.childNodes[cn].tagName.toLowerCase() == "img")
        doc.childNodes[cn].outerHTML = "";
    }

    //alert("doc.childNodes[cn].hasChildNodes(): " + doc.childNodes[cn].hasChildNodes());
    if (doc.childNodes[cn].hasChildNodes()) changeNode(doc.childNodes[cn]);
  }

  /* console.log('translate ' + '#' + translateElement); console.log(jQuery('#' + translateElement).html()); */
      ...
      if (node.childNodes[y].tagName.toLowerCase() == "select") {
        ...
        if (e.onclick != null) {
          ...
          if ( fname.indexOf("EditScript") > -1) {
            ...
            var sreq = 'object oTmp = dom.GetObject( "' + ssid + '" ); \
              if( oTmp ) { if( oTmp.DestinationValueType() == ivtString ) { Write( oTmp.DestinationValue().ToString() ); } }';
            //alert(sreq);
            req.send(sreq);
            ...
            var sreq = 'object tmObj = dom.GetObject("' + ssid + '");';
            if (window.DOMParser) sreq += 'Write("<?xml version=\\"1.0\\"?><!DOCTYPE tm [<!ENTITY amp \\"&#38;\\">]" # ">"); ';
            sreq += 'Write("<tm><zeile1>"); \
              if (tmObj && (tmObj.Type() == OT_CALENDARDP)) { \
                if (tmObj.SunOffsetType() == sotNone) { \
                  if ( (tmObj.CalDuration() == 0) && (tmObj.Time() == 0) ) { \
                    Write("<info>Zeitbereich ganzt&amp;auml;gig</info>"); \
                  } else { \
                    if (tmObj.CalDuration() != 0) { \
                      Write("<info>Zeitbereich von </info>"); \
                      Write("<convertDomTime>" # tmObj.Time() # "</convertDomTime>"); \
                      Write("<info> bis </info><calcEndTime>" # tmObj.Time() # ", " # tmObj.CalDuration() # "</calcEndTime>"); \
                    } else { \
                      Write("<info>Zeitpunkt: </info>"); \
                      Write("<convertDomTime>" # tmObj.Time() # "</convertDomTime>"); \
                    } \
                  } \
                } else { \
                  if (tmObj.SunOffsetType() == sotAfterSunrise) { \
                    Write("<info>Zeitbereich Astrofunktion tags&amp;uuml;ber</info>"); \
                  } \
                  if (tmObj.SunOffsetType() == sotAfterSunset) { \
                    Write("<info>Zeitbereich Astrofunktion nachts</info>"); \
                  } \
                } \
                ...
                integer iType = tmObj.TimerType(); Write("</zeile1><zeile2>"); if (iType == ttCalOnce) { \
                  Write("<info>einmalig am: "); \
                  Write("<convertDomDate>" # sCalRep # "</convertDomDate>"); \
                  } \
                if (iType == ttPeriodic) { \
                  Write("<info>Zeitintervall, alle "); \
                  integer iSelIdx = 0; \
                  string sVal = ""; \
                  sVal = iPeriod; \
                  if ( (iPeriod / (3600)) > 0) { \
                    if( (iPeriod % 3600) == 0 ) { \
                      sVal = iPeriod / (3600); iSelIdx = 0; \
                    } \
                    else { \
                      if( (iPeriod % 60) == 0 ) { \
                  ...
                  if (iSelIdx == 0) { Write(sVal # " Stunden"); } \
                  if (iSelIdx == 1) { Write(sVal # " Minuten"); } \
                  if (iSelIdx == 2) { Write(sVal # " Sekunden"); } \
                } \
                if (iType == ttCalDaily) { \
                  Write("<info>t&amp;auml;glich, "); \
                  if (iRepVal != 0) { \
                    Write("alle " # iRepVal # " Tage"); \
                  } \
                  if (iWdays != 0) { \
                    if (iWdays == 96) { Write("am Wochenende (Sa., So.)"); } \
                    else { Write("jeden Tag, außer am Wochenende (Mo. bis Fr.)"); } \
                  } \
                ...
                if (iType == ttCalWeekly) { \
                  Write("<info>w&amp;ouml;chentlich, "); \
                  if (iRepVal != 0) { \
                    Write("alle " # iRepVal # " Wochen"); \
                  } \
                  else { \
                    Write("jede Woche am: "); \
                  } \
                ...
                if (iType == ttCalMonthly) { \
                  Write("<info>monatlich, "); \
                  if (iWdays == 0) { \
                    Write("am " # iPeriod # ". Tag jedes " # iRepVal # ". Monats"); \
                  } \
                  else { \
                    Write("jeden "); \
                    if (iPeriod == 1) { Write("ersten "); } \
                    if (iPeriod == 2) { Write("zweiten "); } \
                    if (iPeriod == 3) { Write("dritten "); } \
                    if (iPeriod == 4) { Write("vierten "); } \
                    if (iPeriod == 5) { Write("f&amp;uuml;nften "); } \
                    if (iWdays == 1) { Write("Montag "); } \
                    if (iWdays == 2) { Write("Dienstag "); } \
                    if (iWdays == 4) { Write("Mittwoch "); } \
                    if (iWdays == 8) { Write("Donnerstag "); } \
                    if (iWdays == 16) { Write("Freitag "); } \
                    if (iWdays == 32) { Write("Samstag "); }  \
                    if (iWdays == 64) { Write("Sonntag "); } \
                    Write("jedes " # iRepVal # ". Monats"); \
                  } \
                } \
                if (iType == ttCalYearly) { \
                  Write("<info>j&amp;auml;hrlich, "); \
                  if (iWdays == 0) { \
                    Write("jeden " # iPeriod # ". "); \
                    if (iRepVal == 1) { Write("Januar"); } \
                    if (iRepVal == 2) { Write("Februar"); } \
                    if (iRepVal == 3) { Write("M&amp;auml;rz"); } \
                    if (iRepVal == 4) { Write("April"); } \
                    if (iRepVal == 5) { Write("Mai"); } \
                    if (iRepVal == 6) { Write("Juni"); } \
                    if (iRepVal == 7) { Write("Juli"); } \
                    if (iRepVal == 8) { Write("August"); } \
                    if (iRepVal == 9) { Write("September"); } \
                    if (iRepVal == 10) { Write("Oktober"); } \
                    if (iRepVal == 11) { Write("November"); } \
                    if (iRepVal == 12) { Write("Dezember"); } \
                  } \
                  else { \
                    Write("am "); \
                    if (iPeriod == 1) { Write("ersten "); } \
                ...
                Write("<info>Beginn: </info><convertDomDate>" # sBegin # "</convertDomDate>"); \
                if ( (iCalCount == 0) && (tmObj.EndSeconds() == 0) ) { \
                  Write("<info>, kein Enddatum</info>"); \
                } \
                else { \
                  if (iCalCount == 0) { \
                    Write("<info>, endet am: </info><convertDomDate>" # sEnd # "</convertDomDate>"); \
                  } \
                  else { \
                    Write("<info>, endet nach " # iCalCount # " Terminen</info>"); \
                  } \
                } \
                Write("</zeile3></tm>"); \
              }';
...
parseXML = function() {
  if (xml.readyState == 4) startParsing();
};
